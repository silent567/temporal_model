    ...
    def build_model(self):
        ...
        with self.graph.as_default():
            ...
            with _tf.name_scope('ReshapeInput'):
                ...
                self.reshaped_input_data = _tf.reshape(self.perm_expand_input_data,[-1,param.input_channel_num],name='reshaped_input_data')

            self.stacked_dense_out,self.stacked_dense_layers,self.stacked_dense_output = \
                _nn.fc.stackedDenseBlock(self.reshaped_input_data,param.hs,param.layer_num,train_flag=self.train_flag,gated_flag=param.gated_flag,ln_flag=param.ln_flag,dp_flag=param.dp_flag,dp=param.dp,res_flag=param.res_flag,skip_flag=param.skip_flag,activation_func=_tf.nn.relu,name_scope='stackedDenseBlock')

            with _tf.name_scope('PermReshapeStackedDenseout'):
                self.reshaped_stacked_dense_out = _tf.reshape(self.stacked_dense_out,_tf.concat([_tf.shape(self.perm_expand_input_data)[:-1],[param.hs]],axis=0),name='reshaped_stacked_dense_out')
                self.perm_reshaped_stacked_dense_out = _tf.transpose(self.reshaped_stacked_dense_out,perm=[0,2,1,3])

            with _tf.name_scope('BinaryRelationDecoder'):
                self.biconn_dot_out = _tf.matmul(self.perm_reshaped_stacked_dense_out,self.perm_reshaped_stacked_dense_out,transpose_b=True,name='biconn_dot_out')
            ...
            with _tf.name_scope('Loss'):
                ...
                self.l2_loss = _tf.reduce_sum([self.dense_layer.get_l2_loss()] + [l.get_l2_loss() for l in self.stacked_dense_layers],name='l2_loss')
