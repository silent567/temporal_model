...
class _Param(_paramRange.VirtualParam):
    def __init__(self):
        self.model_name = 'softmax+drop+FC'
    ...
    def build_model(self):
        ...
        with self.graph.as_default():
            ...
            with _tf.name_scope('Flat'):
                self.softmax_out_drop_flat = _tf.reshape(self.input_data,[-1,param.input_dim],name='softmax_out_drop_flat')
                self.input_label_flat= _tf.reshape(self.input_label,[-1,param.label_dim],name='input_label_flat')

            with _tf.name_scope('Dense'):
                self.dense_layer = _nn.fc.Dense(param.input_dim,param.label_dim,activation_func=_linear_activation,name_scope='DenseLayer')
                self.logits = self.dense_layer(self.softmax_out_drop_flat)
