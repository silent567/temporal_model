...
class Model(_virtualNeuralNetwork.VirtualNN):
    def _causal_dilated_conv_layer(self,input_tensor,train_flag,dilation_rate,name_scope='CausalDilatedConvLayer'):
        ...
        with _tf.name_scope(name_scope):
            cdconv_layer = _nn.conv.CausalDilatedConvCell([2,input_tensor.shape.as_list()[-1],param.cdconv_hs*(2 if param.gated_flag else 1)],[dilation_rate,],name_scope='cdconv_layer')
            cdconv_out = cdconv_layer(input_tensor)
            if param.gated_flag:
                cdconv_out_candidate = _get_first_half_channel(cdconv_out)
                if param.ln_flag:
                    cdconv_candidate_ln_layer = _nn.norm.LayerNorm([param.cdconv_hs],name_scope='cdconv_candidate_layer_norm_layer')
                ...
                if param.ln_flag:
                    cdconv_gate_ln_layer = _nn.norm.LayerNorm([param.cdconv_hs],name_scope='cdconv_gate_layer_norm_layer')
                ...
                if param.ln_flag:
                    cdconv_ln_layer = _nn.norm.LayerNorm([param.cdconv_hs],name_scope='cdconv_layer_norm_layer')
    ...
    def build_model(self):
        ...
        with self.graph.as_default():
            ...
            self.cdconv_layer,self.cdconv_out = self._causal_dilated_conv_layer(self.input_data,self.train_flag,1,name_scope='CausalConvLayer')
            ...
            with _tf.name_scope('Flat'):
                self.stacked_cdconv_out_flat = _tf.reshape(self.stacked_cdconv_out,[-1,param.cdconv_hs],name='stacked_cdconv_out_flat')
            ...
            with _tf.name_scope('Dense'):
                self.dense_layer = _nn.fc.Dense(param.cdconv_hs,param.label_dim,activation_func=_linear_activation,name_scope='DenseLayer')
